...
int main(int argc, char **argv)
{
  ...
  //{
    double t1 = MPI_Wtime();
    double domXsize = Lx / (Np - n * (Np - 1.0) / (Nx + 1.0));
    ...
    for (int i = 0; i < Ny; i++)
    {
      ...
      for (int k=0;k<Nx;k++)
      { double a=rank * deltax * (Nx - n) + k * deltax, b=i * deltay;
        SolFile << rank * deltax * (Nx - n) + k * deltax << " " << i * deltay << " " << y[i * Nx + k]<<endl;//-a*(1-a)*b*(1-b) << endl;
      }
    }
    SolFile.close();
