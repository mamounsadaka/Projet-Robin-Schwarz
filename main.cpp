...
int main(int argc, char **argv)
{
  ...
  while (!Converged)
  {
    ...
    {
      MPI_Status status;
      MPI_Send(&stencilL[0], Ny, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD);
      MPI_Send(&stencilR[0], Ny, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD);
      MPI_Recv(&stencilL[0], Ny, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &status);
      MPI_Recv(&stencilR[0], Ny, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &status);
    }
    else
    {
      if (rank == 0)
